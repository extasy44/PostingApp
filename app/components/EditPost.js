...
const EditPost = (props) => {
    ...
    switch (action.type) {
      ...
      case "titleCheck":
        ...
        if (action.value.length <= 2) {
          draft.title.hasErrors = true;
          draft.title.message = "Title must have more than 2 characters";
        }
        return;
      case "bodyCheck":
        if (!action.value.trim()) {
        ...
        if (action.value.length <= 10) {
          draft.body.hasErrors = true;
          draft.body.message = "body must have more than 10 characters";
        }
        return;
      case "notFound":
        draft.notFound = true;
      ...
      try {
        ...
        if (response.data) {
          ...
          if (appState.user.username != response.data.author.username) {
            appDispatch({
              type: "flashMessages",
              value: "You do not have permission to edit",
            });
  ...
  return (
    <Page title={`Update ${state.title.value}`}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <textarea
              ...
              onBlur={(e) =>
                dispatch({ type: "bodyCheck", value: e.target.value })
              }
              name="body"
              id="post-body"
